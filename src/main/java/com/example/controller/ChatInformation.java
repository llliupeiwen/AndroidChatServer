...
@WebServlet(name = "ChatInformation", value = "/ChatInformation")
public class ChatInformation extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ChatList chatList = JSON.parseObject(s.toString(), ChatList.class);//是用了反射机制來完成对象的封闭
        //以utf-8解码操作
        String number = URLDecoder.decode(chatList.getNumber(), "utf-8");
        System.out.println(chatList);
        // 去数据库完成用户登录功能
        UserServiceImpl us = new UserServiceImpl();
        //调用登录的方法
        ArrayList<ChatList> ChatList1 = us.informationUser(number);
        if (ChatList1 != null) {
            //将结果返回给客户端	，將結果構建成json數據返回給客戶端
            JSONObject rjson = new JSONObject();
            rjson.put("j1",true);
            rjson.put("json", ChatList1);
